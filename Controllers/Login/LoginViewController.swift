...
class LoginViewController: UIViewController {
    ...
    private let imageView: UIImageView = {
        ...
        imageView.image = UIImage(named: "HiLondonLogo")
    ...
    @objc private func loginButtonTapped() {
        ...
        FirebaseAuth.Auth.auth().signIn(withEmail: email, password: password, completion: { [weak self] authResult, error in
            ...
            guard let result = authResult, error == nil else {
                print("Failed to Login for user: \(email)")
                return
            }
            ...
            print("Loggined in User: \(user)")
            strongSelf.navigationController?.dismiss(animated: true, completion: nil)
        })
    }
    
    func alertUserLoginError() {
        let alert = UIAlertController(title: "Sorry", message: "Please enter both Email and Password to continue.", preferredStyle: .alert)
    ...
    func loginButton(_ loginButton: FBLoginButton, didCompleteWith result: LoginManagerLoginResult?, error: Error?) {
        ...
        facebookRequest.start(completion: { _, result, error in
            ...
            FirebaseAuth.Auth.auth().signIn(with: credential, completion: { [weak self] authResult, error in
                ...
                print("Login Successed by Facebook")
                strongSelf.navigationController?.dismiss(animated: true, completion: nil)
            })
        })
